...
def _init_counter():
    ...
    """)
    cur.execute("SELECT total_visits FROM counters WHERE id = 1")
    row = cur.fetchone()
    if row is None:
        cur.execute("INSERT INTO counters (id, total_visits) VALUES (1, 0)")
    conn.commit()
    conn.close()
...
def _increment_and_get():
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE counters SET total_visits = total_visits + 1 WHERE id = 1")
    conn.commit()
    cur.execute("SELECT total_visits FROM counters WHERE id = 1")
    count = int(cur.fetchone()["total_visits"])
    conn.close()
    return count

# initialize DB at import-time (works on older Flask versions too)
_init_counter()

@app.route('/', methods=['GET', 'POST'])
def submit():
    # increment global counter on every page view (GET and POST)
    visitor_count = _increment_and_get()
